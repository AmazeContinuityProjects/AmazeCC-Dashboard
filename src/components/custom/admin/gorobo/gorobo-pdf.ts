...
// Standard fonts are WinAnsi — the ₹ glyph is not available, so PDFs use "Rs." instead.
...
export function downloadBomPdf(order: GoroboOrderJson) {
  ...
  const cols: Col[] = [
    ...
    { label: 'Unit (Rs.)', width: 90, align: 'right' },
    { label: 'Margin (Rs.)', width: 90, align: 'right' },
  ...
  const rows = order.items.map((line, i) => [
    ...
    line.custom ? '—' : Number(line.margin ?? 0).toLocaleString("en-IN", { minimumFractionDigits: 2 }),
  ...
  drawSummary(state, [
    ...
    { label: `Discount (${order.discountPct}%)`, value: `− ${rs(order.discountAmount)}` },
