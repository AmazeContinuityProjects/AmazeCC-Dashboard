...
export default function FresherResourcesTab() {
 ...
 {error && (
 <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200/50 dark:border-red-800/50 rounded-xl text-red-700 dark:text-red-400 text-sm">{error}</div>
 )}
 {successMsg && (
 <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200/50 dark:border-green-800/50 rounded-xl text-green-700 dark:text-green-400 text-sm">{successMsg}</div>
 )}
 ...
 <h3 className="text-lg font-semibold text-gray-900 dark:text-gray-100 ">New Resource</h3>
 <button onClick={() => setShowAddForm(false)} className="p-1 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-400"><X className="w-5 h-5" /></button>
 </div>
 ...
 {!r.is_active && <span className="text-xs px-2 py-0.5 bg-gray-100 dark:bg-slate-800 rounded-full text-gray-500">Inactive</span>}
 </div>
 {r.description && <p className="text-sm text-gray-500 dark:text-gray-400 mb-1">{r.description}</p>}
 <a href={r.url} target="_blank" rel="noopener noreferrer" className="text-xs text-blue-600 dark:text-blue-400 hover:underline truncate block">{r.url}</a>
 ...
 <button onClick={() => startEdit(r)} className="p-2 rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800 text-gray-500 hover:text-blue-600 transition"><Pencil className="w-4 h-4" /></button>
 <button onClick={() => handleDelete(r.id)} className="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-gray-500 hover:text-red-600 transition"><Trash2 className="w-4 h-4" /></button>
 </div>
 </div>
 )}
 </GlassCard>
