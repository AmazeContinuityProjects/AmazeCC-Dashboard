...
export default function CurriculumPage({ allGradesData, gradesData, marksData, attendance, handleFetchGrades, setActiveSubTab }: {
 ...
 <p className="text-gray-600 dark:text-gray-300 mb-4">No curriculum data available.</p>
 ...
 <Button variant="ghost" size="icon" onClick={() => setActiveSubTab("overview")} className="rounded-full bg-white dark:bg-slate-800 shadow-sm border border-gray-200 dark:border-gray-700 hover:bg-gray-100">
 <ChevronLeft size={20} />
 </Button>
 ...
 <div>
 <h1 className="text-2xl md:text-3xl font-bold text-gray-900 dark:text-gray-100 ">Curriculum Overview</h1>
 <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Track your degree progress across all credit baskets</p>
 </div>
 </div>
 ...
 <Card className="bg-gradient-to-br from-indigo-50 to-blue-50 dark:from-slate-900 dark:to-indigo-950/30 border border-indigo-100 dark:border-indigo-900/40 rounded-2xl shadow-sm overflow-hidden">
 ...
 <span className="text-2xl font-bold text-gray-900 dark:text-gray-100 ">{earnedPct.toFixed(0)}%</span>
 ...
 <h2 className="text-lg font-bold text-gray-900 dark:text-gray-100 mb-4">Total Credits Progress</h2>
 <div className="grid grid-cols-3 gap-3">
 <div className="flex flex-col items-center p-3 rounded-xl bg-white/60 dark:bg-slate-800/50 border border-indigo-100/50 dark:border-indigo-800/30 ">
 <span className="text-[10px] uppercase tracking-wider text-gray-500 font-medium mb-1">Earned</span>
 <span className="text-xl font-bold text-indigo-600 dark:text-indigo-400 ">{totalEarned.toFixed(1)}</span>
 </div>
 <div className="flex flex-col items-center p-3 rounded-xl bg-white/60 dark:bg-slate-800/50 border border-yellow-100/50 dark:border-yellow-800/30 ">
 <span className="text-[10px] uppercase tracking-wider text-gray-500 font-medium mb-1">In Progress</span>
 <span className="text-xl font-bold text-yellow-600 dark:text-yellow-400 ">{totalOngoing.toFixed(1)}</span>
 </div>
 <div className="flex flex-col items-center p-3 rounded-xl bg-white/60 dark:bg-slate-800/50 border border-gray-200/50 dark:border-gray-700/30 ">
 <span className="text-[10px] uppercase tracking-wider text-gray-500 font-medium mb-1">Required</span>
 <span className="text-xl font-bold text-gray-700 dark:text-gray-300 ">{totalRequired.toFixed(1)}</span>
 </div>
 </div>
 {/* Legend */}
...
function ProgressCard({ title, earned, inProgress, required, compact = false }: {
 ...
 return (
 <Card className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm hover:shadow-md transition-shadow">
 <CardContent className={compact ? "p-4" : "p-5"}>
 <div className="flex justify-between items-start mb-3">
 <h3 className={`${compact ? "text-sm" : "text-sm"} font-semibold text-gray-800 dark:text-gray-200 leading-tight pr-2`}>
 {title}
 </h3>
...
function CourseAccordion({ type, courses }: { type: string; courses: EffectiveGradeItem[] }) {
 ...
 return (
 <Card className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
 <button
