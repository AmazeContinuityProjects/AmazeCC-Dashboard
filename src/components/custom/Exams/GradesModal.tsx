...
export default function GradesModal({ allGradesData, GradesData, onClose, handleFetchGrades, marksData, attendance }) {
 ...
 <div className="rounded-2xl shadow-lg w-11/12 max-w-md md:max-w-5xl max-h-[90vh] overflow-y-auto relative bg-white dark:bg-slate-800 ">
 ...
 size="icon"
 onClick={onClose}
 className="top-4 right-4 absolute cursor-pointer hover:bg-gray-200 dark:hover:bg-slate-800 "
 >
...
function GradesDisplay({ allGradesData, data, handleFetchGrades, marksData, attendance }) {
 ...
 <CardHeader>
 <CardTitle className="text-lg font-bold text-gray-900 dark:text-gray-100 ">Grade Distribution <button onClick={handleFetchGrades} className="mt-2 px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition-colors">
 <RefreshCcw className={`w-4 h-4`} />
 </button></CardTitle>
 </CardHeader>
 ...
 <div
 key={grade}
 className="p-2 rounded-lg bg-gray-100 dark:bg-slate-700 text-gray-900 dark:text-gray-100 font-bold"
 >
 <p>{grade}</p>
 ...
 <div className="hidden md:block absolute left-1/2 top-0 bottom-0 w-[1px] bg-gray-300 dark:bg-gray-700 transform -translate-x-1/2" />
