...
export default function MarksHistoryTab({ data }) {
 ...
 {/* Radar Chart */}
 <Card className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-gray-800 rounded-2xl shadow-sm overflow-hidden flex flex-col">
 ...
 return (
 <div key={idx} className="bg-white dark:bg-slate-900 border border-gray-100 dark:border-gray-800 rounded-2xl p-4 shadow-sm">
 <div className="flex justify-between items-start gap-4">
 <div>
 <h4 className="font-bold text-gray-900 dark:text-gray-100 text-sm md:text-base">{course.courseCode} - {course.courseTitle}</h4>
 <div className="flex items-center gap-2 mt-1">
 <span className="inline-block px-2.5 py-0.5 rounded text-[10px] font-medium bg-gray-100 dark:bg-slate-800 text-gray-600 dark:text-gray-300 ">
 {course.courseType}
 </span>
 <span className="inline-block px-2.5 py-0.5 rounded text-[10px] font-bold bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-400 ">
 ...
 <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-800 grid grid-cols-2 md:grid-cols-4 gap-2">
 {course.details.map((detail: any, dIdx: number) => (
 <div key={dIdx} className="bg-gray-50 dark:bg-slate-800/50 rounded-lg p-2 text-center">
 <p className="text-[10px] text-gray-500 dark:text-gray-400 font-semibold uppercase tracking-wider mb-1 line-clamp-1">{detail.component}</p>
 <p className="text-sm font-bold text-gray-800 dark:text-gray-200 ">{detail.scoredMark} <span className="text-xs text-gray-400 font-normal">/ {detail.maxMark}</span></p>
 </div>
 ))}
 </div>
 )}
 ...
 <p className="text-[10px] text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider mb-2">Grade Ranges</p>
 <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-2">
 {Object.entries(course.range).map(([grade, rangeStr]: any, idx) => {
 let colorClass = 'bg-gray-50 text-gray-700 border-gray-200 dark:bg-gray-800/50 dark:text-gray-400 dark:border-gray-700 ';
 if (grade === 'S') colorClass = 'bg-emerald-50 text-emerald-700 border-emerald-200 dark:bg-emerald-900/20 dark:text-emerald-400 dark:border-emerald-800/50 ';
 else if (grade === 'A') colorClass = 'bg-green-50 text-green-700 border-green-200 dark:bg-green-900/20 dark:text-green-400 dark:border-green-800/50 ';
 else if (grade === 'B') colorClass = 'bg-blue-50 text-blue-700 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800/50 ';
 else if (grade === 'C') colorClass = 'bg-indigo-50 text-indigo-700 border-indigo-200 dark:bg-indigo-900/20 dark:text-indigo-400 dark:border-indigo-800/50 ';
 else if (grade === 'D') colorClass = 'bg-purple-50 text-purple-700 border-purple-200 dark:bg-purple-900/20 dark:text-purple-400 dark:border-purple-800/50 ';
 else if (grade === 'E') colorClass = 'bg-orange-50 text-orange-700 border-orange-200 dark:bg-orange-900/20 dark:text-orange-400 dark:border-orange-800/50 ';
 else if (grade === 'F' || grade === 'N') colorClass = 'bg-red-50 text-red-700 border-red-200 dark:bg-red-900/20 dark:text-red-400 dark:border-red-800/50 ';
