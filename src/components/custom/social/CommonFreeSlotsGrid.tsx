...
export default function CommonFreeSlotsGrid({ myAttendance, friends }: { myAttendance: any[], friends: Friend[] }) {
 ...
 const headerClass =
 "border px-1 py-1 bg-[#eef2ff] dark:bg-[#071925] min-w-[100px] text-[11px]";
 const lunchHeaderClass =
 "border px-1 py-1 bg-gray-300 dark:bg-[#162029] min-w-[100px] text-[11px] font-semibold";
 const cellBase =
 ...
 <thead>
 <tr>
 <th className="border px-3 py-2 bg-gray-200 dark:bg-gray-800 ">
 DAY
 </th>
 ...
 {days.map((day) => (
 <tr key={day}>
 <td className="border font-semibold bg-gray-100 dark:bg-[#07101a] ">
 {day}
 </td>

 {beforeLunch.map((p, i) => {
 const { slotLabel, busyCount, freeCount, totalPeople, busyNames, freeNames } = buildCell(day, p);
 let colorClass = "bg-white dark:bg-[#061017] ";
