...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ activeTab, activeSubTab, onLogout }) => {
 ...
 {successMsg && (
 <div className="mb-4 p-4 bg-green-50 dark:bg-green-900/20 border border-green-200/50 dark:border-green-800/50 rounded-xl text-green-700 dark:text-green-400 text-sm backdrop-blur-xl">
 {successMsg}
 </div>
 )}
 {errorMsg && (
 <div className="mb-4 p-4 bg-red-50 dark:bg-red-900/20 border border-red-200/50 dark:border-red-800/50 rounded-xl text-red-700 dark:text-red-400 text-sm backdrop-blur-xl">
 {errorMsg}
 </div>
 )}
 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
 <GlassCard hover>
 <div className={`border-2 border-dashed rounded-2xl p-8 flex flex-col items-center justify-center text-center transition-all ${isUploading ? 'opacity-50 border-gray-300 dark:border-gray-700 ' : 'border-gray-300/50 dark:border-gray-700/50 hover:border-blue-400/50 dark:hover:border-blue-600/50 '}`}>
 <Upload className="h-10 w-10 text-gray-400 dark:text-gray-500 mb-3" />
 <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">Upload buses.json</p>
 <p className="text-xs text-gray-500 dark:text-gray-400 mb-4">This will override the database directly.</p>
 ...
 <p className="text-xs text-gray-500 dark:text-gray-400 mb-3">Export the current database to Excel or JSON format.</p>
 ...
 ? 'bg-red-50 dark:bg-red-900/20 border border-red-200/50 dark:border-red-800/50 text-red-700 dark:text-red-400 '
 : 'bg-green-50 dark:bg-green-900/20 border border-green-200/50 dark:border-green-800/50 text-green-700 dark:text-green-400 '
 }`}>
 {broadcastMsg}
 </div>
 )}
