...
export default function ProfilePage({ currSemesterID, setCurrSemesterID, handleLogin, setIsReloading, handleLogOutRequest, username, password, setPassword, decimalValues, setDecimalValues, loadingScreen, setLoadingScreen, isDayscholarWithBus, setIsDayscholarWithBus, residentialStatus, setResidentialStatus, calendarType, setCalendarType, hideMobileHeader, setHideMobileHeader, reloadAllData, setReloadAllData, isLoggedIn, friendlyName, setFriendlyName }) {
 ...
 <div className="bg-white/60 dark:bg-slate-900/50 backdrop-blur-2xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.1)] border border-white/40 dark:border-gray-700/50 overflow-hidden mb-8">
 {children}
 </div>
 );
 ...
 <div 
 onClick={onClick}
 className={`flex items-center justify-between p-4 ${!noBorder ? 'border-b border-gray-100/50 dark:border-gray-800/50 ' : ''} ${onClick ? 'cursor-pointer hover:bg-white/40 dark:hover:bg-slate-700/30 transition-colors' : ''}`}
 >
 ...
 {trailing ? trailing : (onClick && !isDestructive ? <ChevronRight size={18} className="text-gray-400 " /> : null)}
 </div>
 </div>
 );
 ...
 <p className="text-sm text-gray-500 dark:text-gray-400 font-medium break-words leading-snug">{friendlyName ? `VTOP ID: ${username}` : "AmazeCC User"}</p>
 </div>
 </div>
 </CardContainer>
 ...
 className="flex-1 w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-white/50 dark:bg-slate-900/50 text-gray-800 dark:text-gray-200 "
 >
 ...
 <div className="p-4 border-b border-gray-100 dark:border-gray-800 ">
 <div className="flex flex-col mb-2">
 <label className="font-medium text-gray-900 dark:text-gray-100 ">Update VTOP Credentials</label>
 <span className="text-xs text-gray-500 dark:text-gray-400 mb-3">Change credentials stored inside AmazeCC</span>
 ...
 className="w-full px-3 py-2 border border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-gray-200 "
 />
 ...
 className="w-full px-3 py-2 pr-10 border border-gray-300 dark:border-gray-700 rounded-lg bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-gray-200 "
 />
 <button onClick={() => setShowPassword(!showPassword)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-gray-400 ">
 <Eye size={16} />
 </button>
 </div>
 <Button onClick={() => setPassword([changeUsername, changedPassword])} disabled={!changeUsername || !changedPassword || (changeUsername === username && changedPassword === password)} className="bg-blue-600 hover:bg-blue-700 text-white mt-1">
 ...
 <div className="p-4 border-b border-gray-100 dark:border-gray-800 ">
 <div className="flex flex-col mb-2">
 <label className="font-medium text-gray-900 dark:text-gray-100 ">Residential Status</label>
 <span className="text-xs text-gray-500 dark:text-gray-400 mb-3">Changes which tabs are visible</span>
 <select
 ...
 title={link.title} 
 subtitle={link.desc} 
 trailing={<ExternalLink size={16} className="text-gray-400 " />}
 ...
 <p className="text-xs font-semibold text-gray-400 tracking-wider">MADE BY SUGEETHJSA</p>
 </div>
 ...
 className="w-full flex items-center justify-center gap-2 px-4 py-4 rounded-2xl font-bold bg-red-100 hover:bg-red-200 dark:bg-red-900/20 dark:hover:bg-red-900/40 text-red-600 dark:text-red-500 transition-colors shadow-sm"
 >
 <LogOut size={20} />
 Log Out
 </button>
